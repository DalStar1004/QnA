...
- 기존 게임 로직(드래그 선택, 정답 판정, 캔버스 라인, 타이머)을 최대한 보존하면서 신규 기능을 **역할별 객체(Manager)**로 분리해 추가한다.
...
│                   └─ 이벤트 바인딩(mousedown/mouseenter/mouseup),   │
...
```
사용자 입력(드래그) → GameController.handleSelectionEnd()
...
interface GameState {
  score: number;
  highScore: number;              // StorageManager에서 로드된 값의 캐시
  timeLeft: number;
  isGameActive: boolean;
  currentCategory: string;
  currentLengthLimit: number;     // 2~5
  currentBlockCount: number;
  currentTargetWords: string[];   // 현재 라운드의 정답 후보 2개
...
|-----|------|-------------|
| `wordConnectionGame.highScore` | `string` (숫자 문자열) | 통합 최고 점수 (카테고리 무관). 없으면 0으로 취급 |

> `localStorage` 접근이 불가능한 환경(프라이빗 브라우징 등)을 대비해 `StorageManager`는 `try/catch`로 감싸고, 실패 시 메모리상의 `GameState.highScore`만 사용하도록 degrade한다 (6장 에러 처리 참조).
...
### 5.1 Screen Layout (변경분만 표시)

```
┌────────────────────────────────────────────┐
│  🍭 단어 연결 게임 🍬                          │
├────────────────────────────────────────────┤
│ [⚙️ 설정] [▶ 게임시작] [⏹ 게임종료] [🔊 음소거] │ ← 음소거 버튼 신규 추가
├────────────────────────────────────────────┤
│ [🎯 제시어] [⏱ 남은시간] [🏆 점수(최고:N)]    │ ← 점수 카드에 최고점수 서브텍스트 추가
├────────────────────────────────────────────┤
│  🎲                                         │
│         (블록 보드 + 캔버스 라인)              │
│                                              │
...
│ 🧩 블록 개수   [   16   ]  │
│ 🔤 목표 글자수 [    2   ]  │
│ 📚 카테고리    [ 랜덤 ▾ ]  │ ← 신규 추가 (select: 랜덤 + 사전 카테고리 전체)
│         [완료 ✔]           │
└──────────────────────────┘
...
페이지 로드 → (설정 기본값/카테고리 select 채움) → [게임 시작]
  → 라운드 진행(드래그로 단어 연결) → 정답/오답 피드백 → (콤보 누적)
...
| 상황 | 원인 | 처리 |
|------|------|------|
| 선택한 카테고리+글자수 조합에 단어 2개 미만 | 사전에 해당 길이 단어 부족 | 기존 로직처럼 `showToast(..., "warn")`로 안내 후 시작 취소. 카테고리를 특정 값으로 선택했을 때도 동일하게 검증(랜덤일 때는 유효한 카테고리 중에서만 추첨하던 기존 로직 유지) |
...
- 사용자 입력은 블록 개수/글자수 숫자 입력(`min`/`max` 이미 존재)과 카테고리 select(고정 옵션)뿐이며 외부로 전송되지 않으므로 XSS/Injection 위험 없음
- 서버/인증 없음 — 해당 항목 전부 N/A
- 수집되는 개인 데이터 없음 (localStorage에는 최고 점수 숫자만 저장)
...
| # | 시나리오 | 절차 | 성공 기준 |
|---|---------|------|----------|
| 1 | 글자수별 시작 가능 여부 | 설정에서 목표 글자수를 2/3/4/5로 각각 바꿔가며 게임 시작 | 4가지 글자수 모두 경고 토스트 없이 정상 시작됨 |
| 2 | 카테고리 직접 선택 | 설정에서 특정 카테고리를 선택 후 시작 | 제시어가 선택한 카테고리로 고정됨 |
| 3 | 정답 처리 | 블록을 드래그해 정답 단어 완성 | 점수 +1, 컨페티/사운드 재생, 새 라운드로 전환 |
