...
//
// 혼자 하기 모드(word_connection_game.html)의 모드 2가 이 서버를 거쳐 힌트를 받는다(/api/ai/*).
// 브라우저가 Groq 를 직접 부르지 않고 **서버가 부르는** 이유는 두 가지다.
//   1. 정답이 서버에만 있어야 한다 (Design §1.2). 힌트를 서버가 만들면 정답이 밖으로 나갈 일이 없다.
//   2. API 키가 서버에만 있으면 된다. 브라우저로 키가 나가지 않는다.
//
// 이번엔 혼자 하기 모드 2에서 Gemini와 나란히 고를 수 있는 **선택지 하나**로 들어온다 —
// 멀티플레이가 쓰는 hintGenerator(OllamaHintGenerator/GeminiService, AI_PROVIDER로 고정)는
// 그대로 두고 손대지 않았다. 바깥에서 보는 모습(ensureReady / generateHints / markStale / ping)은
// OllamaHintGenerator 와 똑같이 맞췄다. 달라진 점은 세 가지다.
...
// 대체 힌트를 만드는 판단은 호출부(server/src/presentation/aiRoutes.js)가 한다.
